...
class TicketLifecycleTests(unittest.TestCase):
    ...
    def test_closed_ticket_cannot_be_unassigned(self) -> None:
        ticket = create_ticket(
            TicketCreate(
                title="Closed unassignment guard",
                description="Closed incidents should keep final ownership history stable.",
                visibility=TicketVisibility.public,
                assigned_to_id=self.agent.id,
            ),
            session=self.session,
        ...
        with self.assertRaises(HTTPException) as context:
            unassign_ticket(closed.id, session=self.session, current_user=self.admin)
